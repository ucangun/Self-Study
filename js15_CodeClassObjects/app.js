...
userTodos.forEach((a) => {
  a.userId && console.log(` ${a.completed} --- ${a.title} `);
});
...
userTodos.forEach((a) => {
  a.userId === 1 && console.log(` ${a.completed} --- ${a.title} `);
});
...
const addNewObject = function (userId, id, title, completed) {
  userTodos.push({
    userId: userId,
    id: id,
    title: title,
...
addNewObject(6, 101, "bla bla bla", false);
...
products.forEach(({ price, title, thumbnail }) => {
  document.write(`${title} </br> ${price} </br> <img src=${thumbnail}> </br>`);
});
...
/*
const userId = +prompt("Lütfen bir id giriniz");
const newArray = products.filter(({ id }) => id !== userId);
...
//! fiyatlari indirimli fiyatlar ile güncelleyelim - Update
/*
const newP = products.map(
...
/*
let categoryCounts = products.reduce((counts, product) => {
...
//! NOT Eger sifirdan bir object in icini elimdeki dizideki verilerden bazilari ile doldurmak istiyorsam, öncesinde objem bos oldugu icin dot notation ile (sayilar.category) yapamam. bu yüzden square bracket kullanmaliyim (sayilar[category])
...
customerList.forEach(({ name, phone, address: { city, street, suite } }) =>
  console.log(
    ` name : "${name.toUpperCase()}": , address: "${city}, ${street}, ${suite}", phone: "${phone}" `
  )
);
...
customerList.forEach(({ name, phone }) => {
  let result;
  if (name[0] === "E") {
    result = `Pazartesi aranacaklar...   name: ${name} , phone: ${phone}`;
  } else if (name[0] === "C") {
    result = `Carsamba aranacaklar...    name: ${name} , phone: ${phone}`;
  } else {
    return;
  }
  console.log(result);
...
customerList.forEach(({ id, name, email }) => {
  let output;
  if (id % 2) {
    output = `name: ${name}, email: ${email}, mesaj: 'Yeni çıkan XXX elektronik cihazımızı mutlaka denemelisiniz'`;
  } else {
    output = `name: ${name}, email: ${email}, mesaj: 'Yeni çıkan cıkan kahve yapma makinamızı mutlaka denemelisiniz ' `;
  }
  console.log(output);
});
